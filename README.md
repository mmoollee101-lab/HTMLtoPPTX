...
2. `dom-to-pptx`가 각 요소의 computed style을 측정해 절대좌표 PPTX 박스로 배치합니다.
   ...
   - 소스 비율 자동 감지(16:9/4:3/세로형 등) → 잘림 없이 변환
...
16:9가 아니어도 됩니다. 변환 시 **소스 슬라이드의 비율을 자동 감지**해 그 비율로 PPT를 만듭니다.
- 16:9(1920×1080) → 10×5.625in, 4:3(1024×768) → 10×7.5in, 세로형/와이드도 그대로.
- 특정 비율로 강제하려면 `--aspect 16:9` (또는 `4:3` 등).
- 한 덱 안에서 슬라이드마다 크기가 다르면 첫 슬라이드 기준으로 잡힙니다(보통 전 슬라이드 동일).

## 충실도 팁 (중요)

- **고정 px 권장**: 슬라이드를 고정 px(예: `1920×1080` 또는 `1024×768`)로 작성하세요.
  `vw/vh/%` 가변 단위는 뷰포트(1920×1080) 기준으로 측정돼 의도한 비율과 달라질 수 있습니다.
