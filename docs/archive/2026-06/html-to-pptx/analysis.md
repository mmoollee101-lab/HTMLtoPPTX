...
## 후속 개선 7 (자잘한 충실도 일반화: 인라인 간격·장식 마커)
- **요청**: 후처리한 수준이 앞으로 어떤 HTML에도 적용되도록 일반화.
- **구현(변환 전 DOM 보정, `enhanceFidelity` 기본 ON)**:
  1. 인라인 요소 가로 margin → 공백 문자 환산(예: `±0.1mm`→`±0.1 mm`). 검증: 런이
     `["±0.1"," ","mm"]`로 분리됨.
  2. 그린 장식 의사요소(`::before/::after`, content 비었고 bg/border 있는 정사각형 마커)
     → ●/○/■/□ 글리프로 변환(절대위치 텍스트 박스는 dom-to-pptx가 안정적으로 내보냄).
     검증: slide5=1, slide9=3, slide11=2 마커 글리프 생성. 정사각형 비율 가드로 막대/밑줄 오탐 방지.
- **검증**: 실사용 덱 5 gap + 21 marker, 일반 샘플은 0 marker(텍스트 불릿 ▶는 올바르게 제외),
  양쪽 zip 무결성 OK.
